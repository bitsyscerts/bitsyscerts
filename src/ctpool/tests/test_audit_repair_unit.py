...
from ctpool.audit_repair_strategies import repair_unsupported
...
async def test_apply_repair_increments_attempt_count_unit() -> None:
    ...
    with patch(
        ...
        await apply_repair(finding, session, dry_run=False)

    assert finding.repair_attempt_count == 1
    mock_strategy.assert_awaited_once_with(finding, session)


async def test_apply_repair_dry_run_calls_rollback() -> None:
    """apply_repair calls session.rollback() when dry_run=True."""
    finding = _make_finding(repair_attempt_count=0)
    mock_strategy = AsyncMock(return_value=finding)
    session = AsyncMock()
    ...
    with patch(
        ...
        await apply_repair(finding, session, dry_run=True)

    session.rollback.assert_awaited_once()


async def test_apply_repair_non_dry_run_calls_flush() -> None:
    """apply_repair calls session.flush() when dry_run=False."""
    finding = _make_finding(repair_attempt_count=0)
    mock_strategy = AsyncMock(return_value=finding)
    session = AsyncMock()
