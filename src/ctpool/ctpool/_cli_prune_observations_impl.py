"""Implementation for the prune-observations CLI command.

Exports:
    run_prune_observations — Execute or dry-run the observation prune operation.
"""
...
async def run_prune_observations(
    *,
    dry_run: bool = True,
    ...
    """Delete ct_log_observations rows older than the retention window.

    Args:
        dry_run:        If True (default), only count candidates without deleting.
        retention_days: Override config ``ct_observation_retention_days``.
    ...
    days = (
        ...
        else settings.ct_observation_retention_days
    )
    cutoff = datetime.now(UTC) - timedelta(days=days)
    mode_label = "dry-run" if dry_run else "execute"

    console.print(
        f"[bold]prune-observations[/bold] | mode={mode_label} | "
    ...
    async with factory() as session:
        count_stmt = (
            select(func.count())
            .where(CtLogObservation.observed_at < cutoff)
    ...
    while True:
        ...
        _logger.debug("Deleted %d observation rows (total=%d)", deleted, total_deleted)

    await engine.dispose()
    console.print(f"  Deleted [green]{total_deleted:,}[/green] observation rows.")
