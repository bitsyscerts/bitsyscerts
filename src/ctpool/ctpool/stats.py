...
def _make_log_row(log: CtLogSource) -> tuple[str, str, str, str, str]:
    ...
    if tree_size is not None and cursor is not None and tree_size > 0:
        prog_str = f"{cursor / tree_size * 100:.1f}%"
    else:
        prog_str = "—"
...
def _build_stats_table(
    ...
    table.add_column("Cursor", justify="right", min_width=12)
    table.add_column("Progress", justify="right", min_width=9)
    for log in logs:
